...
- [x] 5.1 Extend the generated-output test suite for the blog index, five-item navigation, current Blog state, canonical URLs, draft exclusion, RSS output, store-link protections, and any available article fixture; verify the complete automated suite passes.
- [x] 5.2 Update the README with Markdown authoring, required frontmatter, feature-image guidance, draft preview, validation, publishing, RSS, canonical-domain, and future-redirect notes; verify every documented command and referenced path matches the implementation.
